#!/bin/sh
#
#	Script Name: Enrollment - Configuration 2.5.sh
#	Requirements:
#		- CocoaDialog
#		- Pashua
#
...
while [ -z "${SUCCESS}" ]; do
	if [ ${ATTEMPTS} -le 3 ]; then
		...
		SUCCESS="NO"
		$CD msgbox --title "Mac Registration" --icon "caution" --text "Error 1" --informative-text "GUI tools failed to install correctly. Contact Justin Phillips at 647-4256" --button1 "OK"
		exit 1
	fi
done
...
if [ "$db" == "1" ]; then
	...
	if [ "$locationFilter" == "" ]; then
		while [ "$locationFilter" == "" ]; do
			$CD msgbox --title "Mac Registration" --icon "stop" --text "This is not a known location number." --informative-text "if you feel this is a mistake, contact Justin Phillips at 647-4256 before proceeding." --button1 "OK"
			registerScreen
		done
	else
	...
	while [ -z "${SUCCESS}" ]; do
	  ...
	  if [ ${ATTEMPTS} -le 3 ]; then
	    ...
	    SUCCESS="NO"
	    $CD msgbox --title "PCPS Mac Enrollment" --icon "caution" --text "Error 2" --informative-text "Binding failed. Contact Justin Phillips at 647-4256" --button1 "OK"
		exit 2
	  fi
	...
	echo "66% Configuring: System Settings..." >&3
	...
	# Install iBoss
	if [ -e "/usr/local/bin/iboss.register.sh" ]; then
		echo "iBoss already installed. Skipping..."
	else
	...
	# Run a final recon
	echo "100% Enrollment complete! Restarting in about 1 minute..." >&3
	$jamfBinary recon
