...
#	Change Log
#		06/27/2017
#			Addied Acceleration & Innovation department
#		05/22/2017
#			Addied Networking department
#		03/15/2017
#			Corrected typo in McLaughlin Middle
#		01/06/2017
#			Removed variable references to "assetTag" and changed to the correct variable "sap"
#		12/10/2016
#			Updated code to reflect changes to the Enrollment AD Binder
#		11/7/2016
#			Changed typo "Chain of Lake Elementary" to "Chain of Lakes Elementary"
#
##################################################
...
# WELCOME DIALOG
WELCOME=`$CD ok-msgbox --title "Active Directory Binder" --text "Welcome to the IST AD Binder" \
--informative-text "The following screens will walk you through binding your Mac to the Polk-FL Domain." \
...
if [ "$WELCOME" == "1" ]; then
	...
	*.title = Active Directory Binder
	...
	cb.tooltip = Cancel Active Directory binding.
	...
	echo "User canceled binding. Exiting..."
	exit 0
fi
...
if [ $cb = 1 ]; then
	...
	if [ "${school}" == "Acceleratation & Innovation" ]; then
	...
	echo "25% Assigning computer to user..." >&3
	echo "Assigning computer to user..." 2>&1
	/usr/local/jamf/bin/jamf recon -endUsername $clientUser


	echo "30% Gathering SAP Number..." >&3
	echo "Gathering SAP number..." 2>&1
	/usr/local/jamf/bin/jamf recon -assetTag $sap
	...
	echo "Submitting details to JSS. This could take a while..." 2>&1
	echo "90% Submitting details to JSS. This could take a while..." >&3
	...
	BINDSUCCESS=`$CD msgbox --title "Active Directory Binder"  --text "Active Directory bind successful." \
	--informative-text "Would you like to logout now?" \
	--button1 "Logout" --button2 "Cancel" --icon notice --no-newline --float`

		if [ "$BINDSUCCESS" == "1" ]; then
